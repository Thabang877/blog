...
if(isset($_POST['submit-edit-blog'])) {
    ...
    if($mainImgUrl == "noupdate") {
        if($altImgUrl == "noupdate") {
            ...
            v_post_path = '$blogPath', v_post_summary = '$blogSummary', v_post_content = '$blogContent', n_post_home_page_placement = '$homePagePlacement', d_date_updated='$date', d_time_updated ='$time'
            WHERE n_blog_post_id = '$blogId'";
        }
        else {
            $sqlUpdateBlog = "UPDATE blog_post SET n_category_id = '$blogCategoryId', v_post_title = '$title', v_post_meta_title = '$metaTitle'
            v_post_path = '$blogPath', v_post_summary = '$blogSummary', v_post_content = '$blogContent', v_alt_image_url = '$altImgUrl', n_post_home_page_placement = '$homePagePlacement', d_date_updated='$date', d_time_updated ='$time'
            WHERE n_blog_post_id = '$blogId'";
        }
    }
            ...
            if ($mainImgUrl != "noupdate") {
                $sqlUpdateBlog = "UPDATE blog_post SET n_category_id = '$blogCategoryId', v_post_title = '$title', v_post_meta_title = '$metaTitle'
                v_post_path = '$blogPath', v_post_summary = '$blogSummary', v_post_content = '$blogContent', v_main_image_url = '$mainImgUrl', n_post_home_page_placement = '$homePagePlacement', d_date_updated='$date', d_time_updated ='$time'
                WHERE n_blog_post_id = '$blogId'";
            }
    }
        else {
            ...
            v_alt_image_url = '$altImgUrl', n_post_home_page_placement = '$homePagePlacement', d_date_updated='$date', d_time_updated ='$time'
            WHERE n_blog_post_id = '$blogId'";
    }
...
function uploadImage($img, $imgName, $imgType, $imgDbColumn) {
    ...
    if($img == "") {
     ...
     if ($_FILES[$imgName]["size"] <= 0) {
            ...
            $sqlGetOldImage = "SELECT ".$imgDbColumn."FROM blog_post WHERE n_post_blog_id='$blogId'";
            ...
            if (!empty($oldImgURL)) {
                $oldImageURLArray = explode("/", $oldImgURL);
                $oldImgName = end($oldImageURLArray);
